...
void parseconf_load_file(const char *path){
	...
	while(fgets(setting_line,sizeof(setting_line),fp)!=NULL){		// 读取一行,验证是否是合法配置行
		//printf("%s\n",setting_line);
		if(strlen(setting_line)==0
			|| setting_line[0]=='#'
			|| str_all_space(setting_line)){		// 如果这行都是空白字符串
				//printf("continue\n");
			continue;
		}
		str_trim_crlf(setting_line);					// 去掉/r/n
...
void parseconf_load_setting(const char *setting){
	...
	//printf("parseconf_load_setting->key=%s,value=%s\n",key,value);
	if(strlen(value)==0){	// 判断该行是否有'='字符
	...
	while(p_uint_setting->p_setting_name!=NULL){
		if(strcmp(key,p_uint_setting->p_setting_name)==0){				// 如果找到键值
			if(value[0]=='0'){
	...
	while(p_str_setting->p_setting_name!=NULL){
		if(strcmp(key,p_str_setting->p_setting_name)==0){				// 如果找到键值
			const char **p_cur_setting=p_str_setting->p_variable;
			if(*p_cur_setting){			// 如果之前有数据就要覆盖
				free((char*)*p_cur_setting);
			}	
			*p_cur_setting=strdup(value);	// 为了函数外面也可以用,因为value是局部变量
			return;	
		}
		p_str_setting++;
	}	
